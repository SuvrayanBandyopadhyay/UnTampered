...
class CustomDataset(Dataset): 
    def __init__(self,orig_path,mask_path,tampered_path,n=128): 
        ...
        for f in tqdm.tqdm(os.listdir(tampered_path)):
            ...
            name = name[:-1] if name.endswith('t') else name
            for ext in (".png", ".jpg", ".jpeg",".tif"):
                candidate = os.path.join(mask_path, f"{name}forged{ext}")
            ...
            mask_img = Image.open(mask_path_full).convert("RGB")
            ...
            binary_mask = ((G>g_thresh)).astype(np.float32)
